...
import { addResizeListener, removeResizeListener } from 'element-ui/lib/utils/resize-event' // '../../../src/utils/resize-event'
  ...
  methods: {
    ...
    toggleRowSelection(row, selected, recordTableSelect) {
      ...
      if (this.useVirtual && this.bigDataCheckbox) {
        ...
        const fale2 = this.store.states.selection.filter(function(item) {
          return item.plChoose === true
        })
        const fale = this.store.states.selection.filter(function(item) {
